...
const AgentDashBoard = () => {
    ...
    const fetchWeatherData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/sensors/data/C8:C9:A3:C8:AE:60"
        );
        ...
        setVoltage(JSON.parse(response.data[0].voltage));
        ...
        const sums = xLabels.map((label) => {
          if (label === formattedHour) {
            ...
            const sum =
              powerValues.reduce((acc, power) => acc + power, 0) / 1000;
            return sum;
          }
          return 0;
        });
  ...
  const containerHeight = status === "threephase" ? "h-[350px]" : "h-[150px]";
  const phaseClass =
    status === "threephase" ? "flex-col space-y-4" : "flex-col";
    ...
    <>
      <div className="w-3/4 flex flex-col p-2">
        ...
        <div className="flex w-full justify-center gap-5">
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex" }}>
                <FaBeer
                  style={{ margin: "10 10px", color: "black", width: "40px" }}
                />
                <FaCoffee
                  style={{ margin: "10 10px", color: "black", width: "40px" }}
                />
                ...
                <div className="dropdown-menu">
                  ...
                  <a
                    className="dropdown-item"
                    href="#"
                    onClick={() => handleItemClick("Montly")}
                  >
                    Monthly
                  </a>
                </div>
            ...
            <BarChart
              ...
              colors={["gray"]}
              series={[
                { data: hourlySums, label: "Current Usage", type: "bar" },
              ]}
          ...
          <div
            className={`w-1/2 p-2 bg-white rounded-lg p-4 ${containerHeight}`}
          >
            <div className="outage">
              ...
              <label className="relative inline-flex items-center mb-1 cursor-pointer">
                ...
                />
                <div
                  className={`w-11 ml-16 h-6 bg-gray-200 rounded-full peer ${
            ...
            {Voltage !== null
              ? Voltage.length > 0 && (
                  <div className={`flex ${phaseClass} h-full`}>
                    {Voltage.map((phase, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-12 flex-1"
                      >
                        <h2 className="shadow-md p-2 rounded-md">
                          Phase {index + 1}:
                        </h2>
                        <ThemeProvider theme={theme}>
                          <Box sx={{ width: "100%" }}>
                            <Slider
                              aria-label={`Voltage ${phase}`}
                              value={phase}
                              min={0}
                              max={300}
      ...
      <div className="w-full md:w-1/4 h-full flex flex-col flex-wrap">
        ...
        <div className="bg-white text-white p-6 rounded-lg shadow-lg max-w-sm">
          <div className="flex justify-between items-center">
            ...
            <div>
              <img
                src="https://img.icons8.com/ios-filled/50/000000/partly-cloudy-day--v1.png"
                alt="Weather Icon"
                className="w-16"
              />
