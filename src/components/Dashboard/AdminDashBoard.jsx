...
const AdminDashBoard = () => {
  ...
  const [threephasecurrent, setThreephasecurrent] = useState([]);
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        if (latestData.phase1 && latestData.phase1.length > 0) {
          ...
          console.warn("No phase 1 data available");
        }
  
        // Phase 2 data processing (similar to Phase 1)
        ...
        // Phase 3 data processing (similar to Phase 1)
        if (latestData.phase3 && latestData.phase3.length > 0) {
          ...
          // Start timer if phase 3 voltage is below 50
          let alertTimer = null;
          const alertDuration = 1000; // Adjust as needed
            ...
            if (!alertTimer) {
              alertTimer = setTimeout(() => {
                ...
                const countdownInterval = setInterval(() => {
                  setCountdownTime((prevTime) => prevTime + 1); // Decrease countdown time
                }, 1000);
                return () => clearInterval(countdownInterval);
              }, alertDuration);
            }
          } else {
            // Clear timer if voltage is above 50
            if (alertTimer) {
              clearTimeout(alertTimer);
              alertTimer = null;
              setCountdownTime(0); // Reset countdown time
            }
          }
  
          // Show alert if phase 3 voltage is between 50 and 150
          if (phase3voltage < 150 && phase3voltage >= 50) {
            Swal.fire({
              position: 'top-end',
              icon: 'error',
              text: 'Voltage of at least one phase is less than 150. Voltage is Dim Or Occurred!',
              showCloseButton: true,
              timer: 5000,
              timerProgressBar: true,
              customClass: {
          ...
          console.warn("No phase 3 data available");
        }
        ...
        if (latestData.three_phase && latestData.three_phase.length > 0) {
          ...
          setThreephasecurrent(latestThreePhaseData.current.toFixed(2))
        } else {
  ...
  const fetchDatas = async () => {
    try {
      ...
      const data = response.data;
      console.log("datttttttttttttttttttttttta", data)
